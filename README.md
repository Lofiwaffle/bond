...
If sign-up fails because tables are missing, run `supabase/bootstrap.sql` in the [SQL editor](https://supabase.com/dashboard/project/melmzlgzfcysbnvtuksv/sql/new). If that errors with `relation "couples" already exists`, the project already has the older tables — paste `supabase/catchup_hosted_current.sql` instead. That one file adds every table a stale project is missing (`daily_actions`, `weekly_ai_summary_prefs`, `couple_prompt_items`) and lets one partner schedule a Together activity, so it clears both `Could not find the table … in the schema cache` and `new row violates row-level security policy for table "partner_signals"`. It is safe to re-run. Under Authentication → URL configuration add `https://lofiwaffle.github.io/bond/**` and `bond://**` (needed for confirmation, Google sign-in, and password reset). Then refresh the app.
