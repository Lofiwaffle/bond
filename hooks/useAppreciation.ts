...
const APPRECIATION_CATEGORIES = ['support', 'humor', 'effort', 'presence', 'other']

export function useAppreciation() {
  const { user, profile, couple, partner } = useAuth()
  const [pendingCategory, setPendingCategory] = useState<'support' | 'humor' | 'effort' | 'presence' | 'other' | null>(null)
  const [pendingMessage, setPendingMessage] = useState<string | null>(null)

  const tapAppreciation = useCallback(async (category: typeof APPRECIATION_CATEGORIES[number], message?: string) => {
